 ...
 */
package se.inera.intyg.intygstjanst.application.citizen.repository;
...
class CitizenCertificatesRepositoryTest {

  @Mock RelationDao relationDao;

  @Mock CertificateDao certificateDao;

  @Mock CitizenCertificateConverter citizenCertificateConverter;

  @InjectMocks CitizenCertificatesRepository citizenCertificatesRepository;
  ...
  private static final CitizenCertificate CONVERTED_CERTIFICATE =
      CitizenCertificate.builder().build();
  private static final List<Certificate> CERTIFICATES =
      List.of(CERTIFICATE_1, CERTIFICATE_2, REVOKED_CERTIFICATE, CERTIFICATE_DB, CERTIFICATE_DOI);
  ...
  class HasCertificates {
    ...
    class Converter {
      ...
      final Relation relation =
          new Relation(CERTIFICATE_ID_1, CERTIFICATE_ID_2, "ERSATT", LocalDateTime.now());
      final Relation otherRelation =
          new Relation("OTHER_ID", "OTHER_ID", "ERSATT", LocalDateTime.now());
      ...
      void shouldSendRelationsForCertificate() {
        ...
        citizenCertificatesRepository.getCertificatesForPatient(PATIENT_ID);
        verify(citizenCertificateConverter, times(2))
            .convert(any(Certificate.class), captor.capture());
