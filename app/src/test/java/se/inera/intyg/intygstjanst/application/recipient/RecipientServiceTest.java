...
class RecipientServiceTest {
  ...
  @Mock private RecipientRepo repo;
  ...
  void testListRecipientsForCerttypeTS() {
    ...
    assertEquals(
        expected, service.listRecipients(new CertificateType(TS_CERTIFICATE_TYPE_DIABETES)));
  }

  @Test
  void testListRecipientsWithUntrusted() {
    // Note: Order matters in the list.
    when(repo.listRecipients())
        .thenReturn(
            Arrays.asList(createUntrustedRecipient(), createFkRecipient(), createTsRecipient()));
    ...
    assertEquals(
        expectedTS, service.listRecipients(new CertificateType(TS_CERTIFICATE_TYPE_DIABETES)));
  }
}
