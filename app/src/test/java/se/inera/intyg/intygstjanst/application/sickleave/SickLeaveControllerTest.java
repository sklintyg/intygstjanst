...
class SickLeaveControllerTest {
  ...
  @Mock private GetSickLeavesService getSickLeavesService;
  @Mock private GetSickLeaveFilterService getSickLeaveFilterService;
  @InjectMocks private SickLeaveController sickLeaveController;
  ...
  private static final List<SickLeaveLengthInterval> SICK_LEAVE_LENGTH_INTERVALS =
      List.of(new SickLeaveLengthInterval(1, 150));
  private static final List<DiagnosKapitel> DIAGNOSIS_CHAPTER =
      List.of(new DiagnosKapitel("A00-B99Vissa infektionssjukdomar och parasitsjukdomar"));
  private static final List<Lakare> DOCTORS = List.of(Lakare.create(DOCTOR_ID, DOCTOR_ID));
  ...
  private static final List<String> OCCUPATION_IDS =
  ...
  class GetActiveSickLeavesForCareUnitTest {
    ...
    void shouldIncludeRequest() {
      ...
      final var getSickLeaveServiceRequestArgumentCaptor =
          ArgumentCaptor.forClass(GetSickLeaveServiceRequest.class);
  ...
  class PopulateFiltersTest {
    ...
    void setUp() {
      ...
      populateFiltersRequestDTO.setMaxDaysSinceSickLeaveCompleted(
    ...
    void shouldIncludeRequest() {
      ...
      final var getSickLeaveFilterServiceRequestArgumentCaptor =
          ArgumentCaptor.forClass(GetSickLeaveFilterServiceRequest.class);
      sickLeaveController.populateFilters(populateFiltersRequestDTO);
      verify(getSickLeaveFilterService)
          .get(getSickLeaveFilterServiceRequestArgumentCaptor.capture());
