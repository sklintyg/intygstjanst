...
import se.inera.intyg.intygstjanst.application.sickleave.dto.Vardgivare;
import se.inera.intyg.intygstjanst.infrastructure.persistence.model.dao.Reko;

@ExtendWith(MockitoExtension.class)
class RekoStatusFilterTest {

  @InjectMocks private RekoStatusFilter rekoStatusFilter;
  ...
  private static Reko getRekoStatus(
      String patientId, String status, LocalDateTime registrationTimestamp) {
  ...
  private static final List<Reko> REKO_STATUSES =
      Arrays.asList(
          ...
          getRekoStatus(
              PATIENT_ID_3, RekoStatusType.REKO_4.toString(), LocalDateTime.now().plusDays(1)),
  ...
  @BeforeEach
  void setup() {
    final var sickLeaveWrongStartDate =
        setUpSickLeave(
            PATIENT_ID_1, SICK_LEAVE_TIMESTAMP.plusDays(10), SICK_LEAVE_TIMESTAMP.plusDays(20));

    final var sickLeaveWrongEndDate =
        setUpSickLeave(
            PATIENT_ID_2, SICK_LEAVE_TIMESTAMP.minusDays(3), SICK_LEAVE_TIMESTAMP.minusDays(2));

    final var sickLeaveWrongPatientId =
        setUpSickLeave(
            WRONG_PATIENT_ID, SICK_LEAVE_TIMESTAMP.minusDays(1), SICK_LEAVE_TIMESTAMP.plusDays(4));

    final var sickLeaveSeveralStatuses =
        setUpSickLeave(
            PATIENT_ID_3, SICK_LEAVE_TIMESTAMP.minusDays(1), SICK_LEAVE_TIMESTAMP.plusDays(4));

    final var sickLeaveWithSameDateAsStartDate =
        setUpSickLeave(PATIENT_ID_4, SICK_LEAVE_TIMESTAMP, SICK_LEAVE_TIMESTAMP.plusDays(4));

    final var sickLeaveWithSameDateAsEndDate =
        setUpSickLeave(PATIENT_ID_5, SICK_LEAVE_TIMESTAMP.minusDays(1), SICK_LEAVE_TIMESTAMP);
