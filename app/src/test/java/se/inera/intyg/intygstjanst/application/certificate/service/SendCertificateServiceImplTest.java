...
class SendCertificateServiceImplTest {
  ...
  @InjectMocks SendCertificateServiceImpl sendCertificateService;

  @BeforeEach
  void setup() throws InvalidCertificateException {
    certificate = getCertificate();
    when(certificateService.getCertificateForCare(anyString())).thenReturn(certificate);
  }
  ...
  class InternalNotificationServiceTest {
    ...
    void shouldSendCertificate() {
      ...
      verify(internalNotificationService)
          .notifyCareIfSentByCitizen(captor.capture(), anyString(), anyString());
    ...
    void shouldSendPatientId() {
      ...
      verify(internalNotificationService)
          .notifyCareIfSentByCitizen(any(Certificate.class), captor.capture(), anyString());
    ...
    void shouldSendHosId() {
      ...
      verify(internalNotificationService)
          .notifyCareIfSentByCitizen(any(Certificate.class), anyString(), captor.capture());
    ...
    class AlreadySent {
      ...
      @BeforeEach
      void setup() {
        when(certificateService.sendCertificate(any(), anyString(), anyString()))
      ...
      void shouldNotCallInternalNotification() {
        ...
        verify(internalNotificationService, times(0))
            .notifyCareIfSentByCitizen(any(Certificate.class), any(), any());
      }
