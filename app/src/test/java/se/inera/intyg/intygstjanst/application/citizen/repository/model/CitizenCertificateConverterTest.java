...
class CitizenCertificateConverterTest {
  ...
  @Mock CitizenCertificateRelationConverter citizenCertificateRelationConverter;

  @InjectMocks CitizenCertificateConverter citizenCertificateConverter;
  ...
  void shouldConvertSentDateWhenSent() {
    ...
    when(certificate.getStates())
        .thenReturn(
            List.of(new CertificateStateHistoryEntry("Id", CertificateState.SENT, timeStamp)));
    final var response = citizenCertificateConverter.convert(certificate, relations);
  ...
  void shouldConvertSentDateWhenNotSent() {
    ...
    when(certificate.getStates())
        .thenReturn(
            List.of(new CertificateStateHistoryEntry("Id", CertificateState.CANCELLED, timeStamp)));
    final var response = citizenCertificateConverter.convert(certificate, relations);
  ...
  class ConversionOfRelationValues {
    ...
    void shouldSendIdToConverter() {
      ...
      verify(citizenCertificateRelationConverter, times(1)).convert(captor.capture(), any());
    ...
    void shouldSendRelationToConverter() {
      ...
      verify(citizenCertificateRelationConverter, times(1)).convert(anyString(), captor.capture());
