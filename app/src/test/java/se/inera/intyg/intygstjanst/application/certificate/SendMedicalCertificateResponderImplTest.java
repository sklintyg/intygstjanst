...
class SendMedicalCertificateResponderImplTest {
  ...
  @Mock private RecipientService recipientService = mock(RecipientService.class);
  ...
  @InjectMocks private SendMedicalCertificateResponderImpl responder;
  ...
  void testSendOk() throws Exception {
    ...
    SendMedicalCertificateResponseType response =
        responder.sendMedicalCertificate(createAttributedURIType(), createRequest());
  ...
  void testAlreadySent() throws Exception {
    ...
    when(certificateService.sendCertificate(PERSONNUMMER, CERTIFICATE_ID, FK_RECIPIENT_ID))
        .thenReturn(SendStatus.ALREADY_SENT);
    ...
    assertEquals(
        "Certificate 'Intygs-id-1234567890' is already sent.", response.getResult().getInfoText());
  ...
  void testSendMedicalCertificateInvalidCertificate() throws Exception {
    ...
    verify(certificateService, never())
        .sendCertificate(PERSONNUMMER, CERTIFICATE_ID, FK_RECIPIENT_ID);
    verifyNoInteractions(statisticsService);
  }
  ...
  void testSendMedicalCertificateCertificateRevoked() throws Exception {
    ...
    assertEquals(INFO, response.getResult().getResultCode());
    assertEquals(
        "Certificate 'Intygs-id-1234567890' has been revoked.", response.getResult().getInfoText());
  ...
  void testSendMedicalCertificateSaknadPatient() throws Exception {
    ...
    SendMedicalCertificateResponseType response =
        responder.sendMedicalCertificate(createAttributedURIType(), invalidRequest);
  ...
  void testSendMedicalCertificatePatientIdUtanBindestreckKorrigeras() throws Exception {
    ...
    doReturn(certificate).when(certificateService).getCertificateForCare(CERTIFICATE_ID);

    SendMedicalCertificateResponseType response =
        responder.sendMedicalCertificate(createAttributedURIType(), request);
