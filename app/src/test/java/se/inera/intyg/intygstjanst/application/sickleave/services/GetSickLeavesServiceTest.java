...
class GetSickLeavesServiceTest {

  @Mock private HsaService hsaService;

  @Mock private GetActiveSickLeaveCertificates getActiveSickLeaveCertificates;

  @Mock private GetSickLeaveCertificates getSickLeaveCertificates;

  @Mock private FilterSickLeaves filterSickLeaves;

  @Mock private RekoStatusDecorator rekoStatusDecorator;

  @InjectMocks private GetSickLeavesService getSickLeavesService;
  ...
  private static final List<SjukfallEnhet> SICK_LEAVES =
      List.of(new SjukfallEnhet(), new SjukfallEnhet(), new SjukfallEnhet());
  private static final List<SjukfallEnhet> FILTERED_SICK_LEAVES =
      List.of(new SjukfallEnhet(), new SjukfallEnhet());
  private static final List<SickLeaveLengthInterval> SICK_LEAVE_LENGTH_INTERVALS =
      List.of(new SickLeaveLengthInterval(1, 365));
  private static final List<String> OCCUPATION_TYPE_IDS =
  ...
  @BeforeEach
  void setUp() {
    getSickLeaveServiceRequestBuilder =
  ...
  class GetSickLeavesTest {
    ...
    void shallIncludeCareProviderId() {
      final var careProviderIdCaptor = ArgumentCaptor.forClass(String.class);
      getSickLeavesService.get(getSickLeaveServiceRequestBuilder.build());
      verify(getSickLeaveCertificates)
          .get(
    ...
    void shallIncludeCareUnitsAndSubUnits() {
      ...
      getSickLeavesService.get(getSickLeaveServiceRequestBuilder.build());
      verify(getSickLeaveCertificates)
          .get(anyString(), unitIdsCaptor.capture(), anyList(), anyInt(), anyInt(), anyString());
      assertEquals(UNIT_IDS, unitIdsCaptor.getValue());
    }
    ...
    void shallIncludePatientIds() {
      ...
      getSickLeavesService.get(getSickLeaveServiceRequestBuilder.build());
      verify(getSickLeaveCertificates)
          .get(anyString(), anyList(), patientIdsCaptor.capture(), anyInt(), anyInt(), anyString());
    ...
    void shallIncludeMaxCertificateGap() {
      ...
      verify(getSickLeaveCertificates)
          .get(
              ...
              anyList(),
              anyList(),
              maxCertificateGapCaptor.capture(),
              anyInt(),
              anyString());
    ...
    void shallIncludeProtectedPersonFilterId() {
      ...
      getSickLeavesService.get(getSickLeaveServiceRequestBuilder.build());
      verify(getSickLeaveCertificates)
          .get(anyString(), anyList(), anyList(), anyInt(), anyInt(), captor.capture());
  ...
  class FilterSickLeavesTest {
    ...
    void shallIncludeSickLeaves() {
      ...
      getSickLeavesService.get(getSickLeaveServiceRequestBuilder.build());
      verify(filterSickLeaves)
          .filter(
              sickLeaveListCaptor.capture(),
              anyList(),
              anyList(),
              anyInt(),
              anyInt(),
    ...
    void shallIncludeSickLeaveLengthIntervals() {
      ...
      verify(filterSickLeaves)
          .filter(
              anyList(),
              sickLeaveLengthIntervalsCaptor.capture(),
              anyList(),
              anyInt(),
              anyInt(),
              any(LocalDate.class),
    ...
    void shallIncludeDiagnosisChapter() {
      ...
      verify(filterSickLeaves)
          .filter(
              anyList(),
              anyList(),
              diagnosisChapterCaptor.capture(),
              anyInt(),
              anyInt(),
              any(LocalDate.class),
              any(LocalDate.class),
