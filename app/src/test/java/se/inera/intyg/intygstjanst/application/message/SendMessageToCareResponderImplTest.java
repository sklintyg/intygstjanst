 ...
 */
package se.inera.intyg.intygstjanst.application.message;
...
import se.inera.intyg.intygstjanst.application.message.validator.SendMessageToCareValidator;
...
class SendMessageToCareResponderImplTest {
  ...
  @Mock private ArendeService sendMessageToCareService;

  @Mock private MonitoringLogService logService;

  @Mock private StatisticsService statisticsService;

  @Mock SoapIntegrationService soapIntegrationService;

  @InjectMocks private SendMessageToCareResponderImpl responder;

  @Test
  void testSendMessage() throws Exception {
    when(soapIntegrationService.sendMessageToCare(
            any(String.class), any(SendMessageToCareType.class)))
        .thenReturn(createClientResponse(ResultTypeUtil.okResult()));

    SendMessageToCareResponseType responseType =
        responder.sendMessageToCare(ENHET_1_ID, buildSendMessageToCareType());
  ...
  void testSendMessageValidationError() throws Exception {
    when(validator.validateSendMessageToCare(any(SendMessageToCareType.class)))
        .thenReturn(List.of("fel"));
    ...
    assertEquals(
        "Validation of SendMessageToCare failed for message with question id 4 and certificate id intygsidextension. [fel]",
        responseType.getResult().getResultText());
    verify(soapIntegrationService, never())
        .sendMessageToCare(any(String.class), any(SendMessageToCareType.class));
  ...
  void testSendMessageClientError() throws Exception {
    ...
    when(soapIntegrationService.sendMessageToCare(
            any(String.class), any(SendMessageToCareType.class)))
        .thenReturn(
            createClientResponse(ResultTypeUtil.errorResult(clientErrorId, clientErrorText)));
    ...
    verify(soapIntegrationService, times(1))
        .sendMessageToCare(any(String.class), any(SendMessageToCareType.class));
    verify(statisticsService, times(0)).messageSent(anyString(), anyString(), anyString());
    verify(sendMessageToCareService, never()).processIncomingMessage((any(Arende.class)));
    verify(logService, never()).logSendMessageToCareReceived(anyString(), anyString());
  ...
  void testSendMessageProcessThrowsException() throws Exception {
    when(soapIntegrationService.sendMessageToCare(
            any(String.class), any(SendMessageToCareType.class)))
        .thenReturn(createClientResponse(ResultTypeUtil.okResult()));
    when(sendMessageToCareService.processIncomingMessage(any(Arende.class)))
        .thenThrow(new PersistenceException("Exception message"));
