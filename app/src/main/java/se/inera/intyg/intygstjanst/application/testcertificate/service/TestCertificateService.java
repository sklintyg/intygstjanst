...
public class TestCertificateService {
  ...
  public TestCertificateEraseResult eraseTestCertificates(LocalDateTime from, LocalDateTime to) {
    ...
    for (var testCertificate : testCertificates) {
      ...
      final var idsToErase =
          getCertificateIdsToErase(testCertificate.getId(), testCertificatesWithRelationsMap);
  ...
  private Map<String, List<Relation>> getCertificateRelations(List<Certificate> testCertificates) {
    final var testCertificatesWithRelationsMap =
        new HashMap<String, List<Relation>>(testCertificates.size());
  ...
  private boolean isCertificatePartOfRelations(
      String id, Map<String, List<Relation>> testCertificatesWithRelations) {
    for (var relationList : testCertificatesWithRelations.values()) {
      for (var relation : relationList) {
        if (id.equalsIgnoreCase(relation.getFromIntygsId())
            || id.equalsIgnoreCase(relation.getToIntygsId())) {
          return true;
        }
      }
    }
  ...
  private List<String> getCertificateIdsToErase(
      String id, Map<String, List<Relation>> testCertificatesWithRelations) {
    ...
    if (testCertificatesWithRelations.containsKey(id)) {
      for (var relation : testCertificatesWithRelations.get(id)) {
        if (relation.getFromIntygsId() != null
            && !idsToErase.contains(relation.getFromIntygsId())) {
