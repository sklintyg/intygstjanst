...
@Service
public class RegisterApprovedReceiversResponderImpl
    implements RegisterApprovedReceiversResponderInterface {

  private static final Logger LOG =
      LoggerFactory.getLogger(RegisterApprovedReceiversResponderImpl.class);
  ...
  @Override
  public RegisterApprovedReceiversResponseType registerApprovedReceivers(
      String s, RegisterApprovedReceiversType request) {
      ...
      try {
        ...
        resultType.setResultText(
            "Request to registerApprovedReceiver contained unknown receiverId '"
                + receiverApprovalStatus.getReceiverId()
                + "'");
        resp.setResult(resultType);
        return resp;
      }
    ...
    for (Recipient knownRecipient : knownRecipients) {
      ...
      for (ReceiverApprovalStatus receiverApprovalStatus : request.getApprovedReceivers()) {
        if (knownRecipient.getId().equalsIgnoreCase(receiverApprovalStatus.getReceiverId())) {
          newReceiver.setApproved(
              receiverApprovalStatus.getApprovalStatus() == ApprovalStatusType.YES);
      ...
      if (!isIncludedInRequest) {
        newReceiver.setApproved(
            knownRecipient.getRecipientType() == CertificateRecipientType.HUVUDMOTTAGARE);
      }
    ...
    if (request.getApprovedReceivers() != null) {
      String receivers =
          request.getApprovedReceivers().stream()
              .map(ar -> ar.getReceiverId() + ": " + ar.getApprovalStatus().name())
              .collect(Collectors.joining(", "));
      monitoringLogService.logApprovedReceiversRegistered(
          receivers, request.getIntygId().getExtension());
    }

    resultType.setResultCode(ResultCodeType.OK);
    resultType.setResultText(
        "Successfully registered "
            + request.getApprovedReceivers().size()
            + " receivers for certificate '"
