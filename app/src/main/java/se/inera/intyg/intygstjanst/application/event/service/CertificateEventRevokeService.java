...
@Service
@Slf4j
public class CertificateEventRevokeService {
  ...
  public CertificateEventRevokeService(
  ...
  private static RevokeCertificateType getRequest(GetCertificateXmlResponse response) {
    ...
    request.setSkickatAv(
        getRevokedBy(
            revokedInformation.getRevokedBy(), response.getUnit(), response.getCareProvider()));
  ...
  private static PersonId getPatientId(PersonIdDTO patientId) {
    ...
    personId.setRoot(
        patientId.getType() == PersonIdTypeDTO.PERSONAL_IDENTITY_NUMBER
            ? PERSON_ID_OID
            : SAMORDNING_ID_OID);
  ...
  private void handleResponse(
      RevokeCertificateResponseType wsResponse, GetCertificateXmlResponse xmlResponse) {
