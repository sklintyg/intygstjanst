...
public class ListSickLeavesForCareResponderImpl implements ListSickLeavesForCareResponderInterface {
  ...
  @Autowired private SjukfallEngineService sjukfallEngineService;

  @Autowired private SjukfallCertificateDao sjukfallCertificateDao;

  @Autowired private SjukfallConverter sjukfallConverter;

  @Autowired private SjukfallCertificateConverter sjukfallCertificateConverter;

  @Override
  @PerformanceLogging(
      eventAction = "list-sick-leaves",
      eventType = MdcLogConstants.EVENT_TYPE_ACCESSED)
  public ListSickLeavesForCareResponseType listSickLeavesForCare(
      String logicalAddress, ListSickLeavesForCareType params) {
    ...
    if (params.getEnhetsId() == null || StringUtils.isEmpty(params.getEnhetsId().getExtension())) {
      throw new IllegalArgumentException(
          "Request to ListSickLeavesForCare is missing required parameter 'enhets-id'");
    }

    if (params.getMaxDagarMellanIntyg() < 0) {
      throw new IllegalArgumentException(
          "Request to ListSickLeavesForCare has invalid value for parameter 'maxDagarMellanIntyg', "
              + "must be >= 0, was "
              + params.getMaxDagarMellanIntyg());
    }

    int maxSjukskrivningslangd =
        params.getMaxSjukskrivningslangd() != null ? params.getMaxSjukskrivningslangd() : MAX_LEN;
    int minstaSjukskrivningslangd =
        params.getMinstaSjukskrivningslangd() != null ? params.getMinstaSjukskrivningslangd() : 0;
    ...
    IntygParametrar sjukfallEngineParams =
        new IntygParametrar(params.getMaxDagarMellanIntyg(), LocalDate.now());
    List<SjukfallEnhet> sjukfall =
        sjukfallEngineService.beraknaSjukfallForEnhet(intygDataList, sjukfallEngineParams);

    // Perform post-processing filtering of sjukskrivningslängder and läkare.
    List<HsaId> lakareList =
        params.getPersonalId().stream().filter(Objects::nonNull).collect(Collectors.toList());
    sjukfall =
        sjukfall.stream()
    ...
    // Transform the output of the sjukfallengine into rivta TjK format and build the response
    // object.
