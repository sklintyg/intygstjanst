...
@Service
public class SendMedicalCertificateResponderImpl
    implements SendMedicalCertificateResponderInterface {

  private static final Logger LOGGER =
      LoggerFactory.getLogger(SendMedicalCertificateResponderImpl.class);
  ...
  public SendMedicalCertificateResponseType sendMedicalCertificate(
    ...
    try {
      ...
      // According to the TKB, SendMedicalCertificate is only used to send certificates to
      // Försäkringskassan.
      Recipient recipient = recipientService.getPrimaryRecipientFkassa();

      // Send certificate to recipient
      SendStatus status =
          certificateService.sendCertificate(personnummer.orElse(null), certId, recipient.getId());

      if (status == SendStatus.ALREADY_SENT) {
        LOGGER.info(certId + " already sent to" + recipient.getId());
        response.setResult(
            ResultOfCallUtil.infoResult("Certificate '" + certId + "' is already sent."));
      } else {
      ...
      LOGGER.info(
          String.format(
              "Tried to send certificate '%s' for patient '%s' but certificate does not exist",
              certId, personnummerHash));

      response.setResult(
          ResultOfCallUtil.failResult(
              String.format(
                  "No certificate '%s' found to send for patient '%s'.",
                  certId, personnummerHash)));
      ...
      LOGGER.info(
          String.format(
              "Tried to send certificate '%s' for patient '%s' which is revoked",
              certId, personnummerHash));

      response.setResult(
          ResultOfCallUtil.infoResult(String.format("Certificate '%s' has been revoked.", certId)));
      ...
      LOGGER.error(
          VALIDATION,
          String.format(
              "Validation error found for send certificate '%s' issued by '%s' for patient '%s': %s",
      ...
      String message =
          cause instanceof ExternalServiceCallException ? cause.getMessage() : ex.getMessage();
      ...
      response.setResult(
          ResultOfCallUtil.failResult(
              String.format(
                  "Certificate '%s' couldn't be sent to recipient because it is a test certificate",
                  certId)));
      return response;
    }
  }
  ...
  private String safeGetCertificateId(SendMedicalCertificateRequestType request) {
    ...
    if (request.getSend() != null
        && request.getSend().getLakarutlatande() != null
  ...
  protected Optional<Personnummer> safeGetCivicRegistrationNumber(
  ...
  private String getPersonnummerHash(
      Optional<Personnummer> personnummer, SendMedicalCertificateRequestType request) {
    // If personnnummer is not present, id from request can be returned since it is not valid
    return personnummer.isPresent()
        ? hashUtility.hash(personnummer.get().getPersonnummer())
        : getPatientId(getPatient(request));
  }
}
