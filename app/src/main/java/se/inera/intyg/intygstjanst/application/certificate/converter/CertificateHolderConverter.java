...
public class CertificateHolderConverter {
  ...
  public CertificateHolder convert(
      Intyg intyg,
      String type,
    ...
    certificateHolder.setCertificateRelation(
        convertRelation(intyg.getIntygsId().getExtension(), intyg.getRelation()));
  ...
  private CertificateRelation convertRelation(String intygsId, List<Relation> relations) {
    if (relations != null && relations.size() > 0) {
      return new CertificateRelation(
          intygsId,
          relations.get(0).getIntygsId().getExtension(),
          RelationKod.fromValue(relations.get(0).getTyp().getCode()),
          LocalDateTime.now());
    }
    return null;
  }
