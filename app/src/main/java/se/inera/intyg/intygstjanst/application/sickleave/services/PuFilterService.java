...
public class PuFilterService {
  ...
  public PuFilterService(PUService puService, HashUtility hashUtility) {
    this.puService = puService;
    this.hashUtility = hashUtility;
  }

  public void enrichWithPatientNameAndFilter(
      List<IntygData> sickLeaves, String protectedPersonFilterId) {
    ...
    while (i.hasNext()) {
      ...
      if (pnr.isEmpty()) {
        i.remove();
        log.warn(
            "Problem parsing personnummer returned by PU service. Removing from list of sjukfall.");
        continue;
      }
      ...
      if (patientFound || patientNotFound) {
        if (patientFound && personSvar.getPerson().avliden()) {
          ...
          final var updatedName =
              patientNotFound ? SEKRETESS_SKYDDAD_NAME_UNKNOWN : SEKRETESS_SKYDDAD_NAME_PLACEHOLDER;
          item.setPatientNamn(updatedName);

          if (protectedPersonFilterId == null
              || !protectedPersonFilterId.equals(item.getLakareId())) {
            i.remove();
          }
  ...
  private Map<Personnummer, PersonSvar> fetchPersons(List<IntygData> sjukfallList) {
    ...
    final var personSvarMap =
        puService.getPersons(this.getPersonnummerListFromIntygDataList(sjukfallList));
  ...
  private List<Optional<Personnummer>> getPersonnummerListOfOptionalsFromList(
      List<IntygData> list) {
    return list.stream()
  ...
  private Personnummer getPersonnummer(String pnr) {
    final var personnummer =
        Personnummer.createPersonnummer(pnr)
