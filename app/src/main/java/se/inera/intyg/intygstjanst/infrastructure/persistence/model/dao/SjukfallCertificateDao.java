...
@Repository
public class SjukfallCertificateDao {

  private static final Logger LOG = LoggerFactory.getLogger(SjukfallCertificateDao.class);
  ...
  // An intyg is excluded from being part of a Sjukfall if there is a relation KOMPLT or ERSATT to
  // it,
  // unless the intyg replacing or complementing it (e.g. the "from") has been revoked.
  private static final String EXCLUDE_REPLACED_INTYG_QUERY =
      "SELECT TO_INTYG_ID FROM RELATION r "
          + "INNER JOIN SJUKFALL_CERT sc ON sc.ID = r.TO_INTYG_ID AND "
          + "r.RELATION_KOD IN "
          + "     ('"
          + RelationKod.ERSATT.value()
          + "','"
          + RelationKod.KOMPLT.value()
          + "') "
          + "JOIN SJUKFALL_CERT sc2 ON sc2.ID = r.FROM_INTYG_ID " // Makes sure the intyg that
          // replaces/complemements
  ...
  @PerformanceLogging(
      eventAction = "find-active-sick-leaves",
      eventType = EVENT_TYPE_ACCESSED,
      eventCategory = EVENT_CATEGORY_DATABASE)
  public List<SjukfallCertificate> findActiveSjukfallCertificate(
      String careGiverId,
      List<String> unitIds,
    ...
    final var patientsWithActiveCertificates =
        patientsWithActiveCertificates(careGiverId, unitIds, activeDate, recentlyClosed);
    final var replacedOrComplementedCertificates =
        replacedOrComplementedIntygForPersonnummerList(patientsWithActiveCertificates);
    ...
    final var sjukfallCertificateTypedQuery =
        entityManager
            .createQuery(queryStrBuilder.toString(), SjukfallCertificate.class)
    ...
    if (recentlyClosed != null) {
      sjukfallCertificateTypedQuery.setParameter(
          "recentlyClosed", recentlyClosed.format(DateTimeFormatter.ISO_DATE));
    }

    if (isNotEmpty(replacedOrComplementedCertificates)) {
      sjukfallCertificateTypedQuery.setParameter(
          "replacedOrComplementedCertificates", replacedOrComplementedCertificates);
    }
  ...
  @PerformanceLogging(
      eventAction = "find-all-sick-leaves",
      eventType = EVENT_TYPE_ACCESSED,
      eventCategory = EVENT_CATEGORY_DATABASE)
  public List<SjukfallCertificate> findAllSjukfallCertificate(
      String careGiverId, List<String> unitIds, List<String> patientIds) {
    return querySjukfallCertificatesForUnitsAndPersonnummer(
        careGiverId, unitIds, patientIds, null, false);
  }

  @PerformanceLogging(
      eventAction = "find-active-sick-leaves-care-unit",
      eventType = EVENT_TYPE_ACCESSED,
      eventCategory = EVENT_CATEGORY_DATABASE)
  public List<SjukfallCertificate> findActiveSjukfallCertificateForCareUnits(
    ...
    final var personNummerList =
        patientsWithActiveCertificates(careGiverHsaId, careUnitHsaIds, today, recentlyClosed);

    return querySjukfallCertificatesForUnitsAndPersonnummer(
        careGiverHsaId,
        careUnitHsaIds,
        personNummerList,
        recentlyClosed.format(DateTimeFormatter.ISO_DATE),
        false);
  }

  private List<String> patientsWithActiveCertificates(
  ...
  @PerformanceLogging(
      eventAction = "find-active-sick-leaves-staff-care-unit",
      eventType = EVENT_TYPE_ACCESSED,
      eventCategory = EVENT_CATEGORY_DATABASE)
  public List<SjukfallCertificate> findActiveSjukfallCertificateForPersonOnCareUnits(
    ...
    String recentlyClosed =
        LocalDate.now().minusDays(maxDagarSedanAvslut).format(DateTimeFormatter.ISO_DATE);
    String query = BASE_QUERY + " AND sc.civicRegistrationNumber = :civicRegistrationNumber";

    List<String> personNummerList =
        entityManager
    ...
    return querySjukfallCertificatesForUnitsAndPersonnummer(
        careGiverHsaId, careUnitHsaIds, personNummerList, recentlyClosed, false);
  }

  @PerformanceLogging(
      eventAction = "find-active-sick-leaves-staff",
      eventType = EVENT_TYPE_ACCESSED,
      eventCategory = EVENT_CATEGORY_DATABASE)
  ...
  public void revoke(String id) {
    ...
    List<SjukfallCertificate> resultList =
        entityManager
            .createQuery(
                "SELECT sc FROM SjukfallCertificate sc " + "WHERE sc.id=:id",
  ...
  public void eraseTestCertificates(List<String> ids) {
    final var sjukfallCertificateList =
        entityManager
            .createQuery(
                "SELECT sc FROM SjukfallCertificate sc " + "WHERE sc.id IN (:ids)",
                SjukfallCertificate.class)
  ...
  public int eraseCertificates(List<String> certificateIds, String careProviderId) {
    ...
    for (var sjukfallCertificate : sjukfallCertificates) {
      ...
      LOG.debug(
          "SjukfallCertificate with id {} from care provider {} was successfully erased.",
          sjukfallCertificate.getId(),
          careProviderId);
    }
  ...
  public List<String> findSickLeavesStoredInCS(List<String> sickLeaveIds) {
    final var sickLeavesStoredInIT =
        entityManager
            .createQuery("SELECT c.id FROM Certificate c WHERE c.id IN (:ids)", String.class)
            .setParameter("ids", sickLeaveIds)
            .getResultList();

    return sickLeaveIds.stream().filter(id -> !sickLeavesStoredInIT.contains(id)).toList();
  }
  ...
  private List<SjukfallCertificate> querySjukfallCertificatesForPersonnummer(String personnummer) {
    ...
    TypedQuery<SjukfallCertificate> query =
        entityManager
    ...
    if (isNotEmpty(replacedOrComplementedIntygsIdList)) {
      query =
          query.setParameter(
              "replacedOrComplementedIntygsIdList", replacedOrComplementedIntygsIdList);
    }
  ...
  private List<SjukfallCertificate> querySjukfallCertificatesForUnitsAndPersonnummer(
    ...
    List<String> personNummerList =
        pnrList.stream().distinct().sorted().collect(Collectors.toList());

    if (LOG.isDebugEnabled()) {
      LOG.debug(
          "Get personnr with active intyg on enhet {} (with mottagningar) returned {} items.",
          careUnitHsaIds,
    ...
    // Next, fetch a list of all replaced/complemented intygsId for the patients in the list
    List<String> replacedOrComplementedIntygsIdList =
        replacedOrComplementedIntygForPersonnummerList(personNummerList);
    ...
    // Finally, fetch all SjukfallCertificates for these persons on the designated units, removing
    // any sjukfall from
    // the replaced or complemented list.
    List<SjukfallCertificate> resultList = query.getResultList();
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          "Read {} SjukfallCertificate for belonging to unit {}",
          resultList.size(),
          careUnitHsaIds);
    }
  ...
  private boolean isNotEmpty(List<String> replacedOrComplementedIntygsIdList) {
    return replacedOrComplementedIntygsIdList != null
        && replacedOrComplementedIntygsIdList.size() > 0;
  }

  @SuppressWarnings("unchecked")
  private List<String> replacedOrComplementedIntygForPersonnummerList(List<String> pnrList) {

    // Note: This MUST be done using native SQL unless we upgrade to Hibernate 5.1+ or later where
    // JOINs between
    // unmapped relations are possible.
    return entityManager
        .createNativeQuery(EXCLUDE_REPLACED_INTYG_QUERY)
