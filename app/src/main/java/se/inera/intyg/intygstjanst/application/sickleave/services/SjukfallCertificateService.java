...
@Service
public class SjukfallCertificateService {

  private static final Logger LOG = LoggerFactory.getLogger(SjukfallCertificateService.class);
  ...
  private final List<String> allowedIntygsTyper =
      Arrays.asList(Fk7263EntryPoint.MODULE_ID, LisjpEntryPoint.MODULE_ID);
  ...
  public boolean created(Certificate certificate) {
    ...
    try {
      ...
      // NOTE: See INTYG-7275 - Code below doesn't automatically handle intyg versions as it's not
      // part of the ModuleApi framework.
      switch (certificate.getType()) {
        case Fk7263EntryPoint.MODULE_ID:
          if (certificateToSjukfallCertificateConverter.isConvertableFk7263(utlatande)) {
            sjukfallCertificateDao.store(
      ...
      LOG.error(
          "Could not construct sjukfall certificate from intyg, ModuleNotFoundException: {}",
          e.getMessage());
      return false;
    } catch (ModuleException e) {
      LOG.error(
          "Could not construct sjukfall certificate from intyg, ModuleException: {}",
          e.getMessage());
      return false;
    }
  }
  ...
  public boolean revoked(Certificate certificate) {
    ...
    try {
      ...
      LOG.debug(
          "Will not mark SjukfallCert {} as deleted. Is of unsupported intygstyp, is smittskydd or does not"
              + " have a diagnoseCode.",
          certificate.getId());
      return false;
