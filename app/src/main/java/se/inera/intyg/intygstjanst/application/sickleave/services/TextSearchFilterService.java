...
@Service
public class TextSearchFilterService {
  ...
  private List<SjukfallEnhet> filterOnTextSearch(
      List<SjukfallEnhet> sickLeaves, String textSearch) {
  ...
  private boolean searchMatchesRekoStatus(SjukfallEnhet sickLeave, String textSearch) {
    return matches(
        sickLeave.getRekoStatus() != null
            ? sickLeave.getRekoStatus().getStatus().getName()
            : NO_ONE,
        textSearch);
  }

  private boolean searchMatchesOccupation(SjukfallEnhet sickLeave, String textSearch) {
    return sickLeave.getSysselsattning().stream()
        .anyMatch(occupation -> matches(occupation, textSearch));
  }
  ...
  private boolean searchMatchesSickLeavePeriod(SjukfallEnhet sickLeave, String textSearch) {
    return matches(sickLeave.getStart().toString(), textSearch)
        || matches(sickLeave.getSlut().toString(), textSearch);
  }
  ...
  private String getDiagnosis(SjukfallEnhet sickLeave) {
    final var diagnosis =
        new StringBuilder()
            ...
            .append(" ")
            .append(
                diagnosisDescriptionService.getDiagnosisDescriptionFromSickLeave(
                    sickLeave.getDiagnosKod().getCleanedCode()))
