...
import se.inera.intyg.intygstjanst.testability.service.TestabilityRegisterCertificate;
import se.inera.intyg.intygstjanst.testability.service.TestabilityRevokeCertificate;
import se.inera.intyg.intygstjanst.testability.service.TestabilitySendCertificate;
...
public class IntegrationTestUtil {
  ...
  public IntegrationTestUtil(
      ...
      TestabilityRevokeCertificate testabilityRevokeCertificate,
  ...
  public void registerCertificateTestabilityCreate(
      TestabilityConfigProvider testabilityConfigProvider) {
    ...
    final var relation =
        testabilityConfigProvider.getRelationsId() != null
            ? getRelation(
                ...
                templateGroup,
                testabilityConfigProvider.getRelationKod())
            : "";
    final var workCapacities = getWorkCapacities(testabilityConfigProvider, templateGroup);
    final var diagnosisCodes =
        getDiagnosis(testabilityConfigProvider.getDiagnosisCode(), templateGroup);
    ST requestTemplate = templateGroup.getInstanceOf("requestParameterizedTestabilityCreate");
    addFieldsFromTestabilityConfigProvider(
        requestTemplate, testabilityConfigProvider, relation, workCapacities, diagnosisCodes);
    executeRegisterCertificate(requestTemplate);
    updateCertificateStatus(testabilityConfigProvider);
  }

  private static List<String> getWorkCapacities(
      TestabilityConfigProvider testabilityConfigProvider, STGroupFile templateGroup) {
  ...
  private void updateCertificateStatus(TestabilityConfigProvider testabilityConfigProvider) {
    if (testabilityConfigProvider.isSend()) {
      testabilitySendCertificate.sendCertificate(
          testabilityConfigProvider.getPatientId(), testabilityConfigProvider.getCertificateId());
    }
    if (testabilityConfigProvider.isRevoked()) {
      testabilityRevokeCertificate.revokeCertificate(
          testabilityConfigProvider.getPatientId(), testabilityConfigProvider.getCertificateId());
    }
  }

  private static void addFieldsFromTestabilityConfigProvider(
      ST requestTemplate,
      TestabilityConfigProvider testabilityConfigProvider,
      String relation,
      List<String> workCapacities,
    ...
    requestTemplate.add(
        "signedAndSentDateTime", dateTimeAsStr(testabilityConfigProvider.getSignTimestamp()));
    requestTemplate.add("careUnitName", testabilityConfigProvider.getCareUnitName());
    applyToFromDatesToRequestTemplate(
        requestTemplate,
        testabilityConfigProvider.getFromDays(),
        testabilityConfigProvider.getToDays());
  }
  ...
  private static String getRelation(
      String relationId, STGroupFile templateGroup, RelationKod relationKod) {
  ...
  private static String getWorkCapacity(
      String workCapacity, STGroupFile templateGroup, int fromDays, int toDays, int instance) {
  ...
  private static void applyToFromDatesToRequestTemplate(
      ST requestTemplate, int fromDaysRelativeToNow, int toDaysRelativeToNow) {
    requestTemplate.add(
        "fromDate",
        LocalDate.now()
            .plusDays(fromDaysRelativeToNow)
            .format(DateTimeFormatter.ofPattern("yyyy-MM-dd")));
    requestTemplate.add(
        "toDate",
        LocalDate.now()
            .plusDays(toDaysRelativeToNow)
            .format(DateTimeFormatter.ofPattern("yyyy-MM-dd")));
  }
