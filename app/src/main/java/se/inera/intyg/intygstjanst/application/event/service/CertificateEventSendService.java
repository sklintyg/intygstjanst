...
public class CertificateEventSendService {
  ...
  public void send(GetCertificateXmlResponse xmlResponse, String xml) {
    try {
      final var logicalAddress =
          recipientService.getRecipient(xmlResponse.getRecipient().getId()).getLogicalAddress();
  ...
  private void handleResponse(
      RegisterCertificateResponseType wsResponse, GetCertificateXmlResponse xmlResponse) {
    ...
    if (wsResponse == null
      ...
      throw new IllegalStateException(
          getResultNullMessage(certificateId, certificateType, recipient));
    }
    ...
    if (result == ResultCodeType.ERROR) {
      throw new IllegalStateException(
          getErrorMessage(certificateId, certificateType, recipient, message));
    }
  }

  private String getResultNullMessage(
      String certificateId, String certificateType, String recipient) {
    return String.format(
        "Send certificate received null result for certificate '%s' of type '%s' sent to recipient '%s'.",
        certificateId, certificateType, recipient);
  }

  private String getInfoMessage(
      String certificateId, String certificateType, String recipient, String message) {
    return String.format(
        "ResultCode INFO received when sending certificate '%s' of type '%s' to recipient '%s'. Info message: '%s'.",
  ...
  private String getErrorMessage(
      String certificateId, String certificateType, String recipient, String message) {
    return String.format(
        "ResultCode ERROR received when sending certificate '%s' of type '%s' to recipient '%s'. Error message: '%s'.",
        certificateId, certificateType, recipient, message);
  }
}
