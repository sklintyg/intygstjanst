...
public class FilterSickLeaves {
  ...
  public FilterSickLeaves(
      DiagnosisChapterService diagnosisChapterService,
      CalculatePatientAgeService calculatePatientAgeService,
  ...
  public List<SjukfallEnhet> filter(
      List<SjukfallEnhet> sickLeaveList,
    ...
    return sickLeaveList.stream()
        ...
        .filter(
            sickLeave ->
                filterOnSickLeaveEndDate(sickLeave, fromSickLeaveEndDate, toSickLeaveEndDate))
  ...
  private boolean filterOnRekoStatuses(SjukfallEnhet sickLeave, List<String> rekoStatusTypeIds) {
    ...
    return rekoStatusTypeIds.stream()
        .anyMatch(
            (rekoStatus) ->
                (sickLeave.getRekoStatus() == null
                        && rekoStatus.equals(RekoStatusType.REKO_1.toString()))
                    || (sickLeave.getRekoStatus() != null
                        && sickLeave.getRekoStatus().getStatus().getId().equals(rekoStatus)));
  }
  ...
  private boolean filterOnOccupation(SjukfallEnhet sickleave, List<String> occupationTypeIds) {
    ...
    return occupationTypeIds.stream()
        ...
        .anyMatch(
            occupationType ->
                sickleave.getSysselsattning().stream()
  ...
  private boolean filterOnDoctorIds(SjukfallEnhet sickLeave, List<String> doctorIds) {
    ...
    return doctorIds.stream()
        .anyMatch((doctorId) -> sickLeave.getLakare().getId().equals(doctorId));
  }

  private boolean filterOnSickLeaveLengthIntervals(
      SjukfallEnhet sickLeave, List<SickLeaveLengthInterval> sickLeaveLengthIntervals) {
  ...
  private boolean filterOnDiagnosisChapters(
      SjukfallEnhet sickLeave, List<DiagnosKapitel> diagnosisChapters) {
    if (diagnosisChapters == null || diagnosisChapters.isEmpty()) {
      return true;
    }
    return diagnosisChapters.contains(
        diagnosisChapterService.getDiagnosisChaptersFromSickLeave(sickLeave));
  }

  private boolean filterOnPatientAge(
      SjukfallEnhet sickLeave, Integer patientAgeFrom, Integer patientAgeTo) {
  ...
  private boolean filterOnSickLeaveEndDate(SjukfallEnhet sickLeave, LocalDate from, LocalDate to) {
    return (from == null || isBeforeOrEquals(from, sickLeave.getSlut()))
        && (to == null || isAfterOrEquals(to, sickLeave.getSlut()));
  }
