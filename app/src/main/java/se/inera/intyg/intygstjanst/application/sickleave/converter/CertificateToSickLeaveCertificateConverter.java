...
public class CertificateToSickLeaveCertificateConverter {
  ...
  public SickLeaveCertificate convertAg7804(Certificate certificate, Utlatande statement) {

    if (!(statement instanceof Ag7804UtlatandeV1)) {
      throw new IllegalArgumentException(
          "Cannot convert " + statement.getClass().getName() + " to SickLeaveCertificate");
    }

    var typedStatement = (Ag7804UtlatandeV1) statement;

    return buildSickLeaveCertificateCommon(
            certificate, typedStatement.getGrundData(), typedStatement.getDiagnoser())
        .workCapacityList(buildWorkCapacityList(typedStatement.getSjukskrivningar()))
        .occupation(
            typedStatement.getSysselsattning() != null
                ? typedStatement.getSysselsattning().stream()
  ...
  public SickLeaveCertificate convertAg114(Certificate certificate, Utlatande statement) {
    ...
    return buildSickLeaveCertificateCommon(
            certificate, typedStatement.getGrundData(), typedStatement.getDiagnoser())
        .workCapacityList(
            buildWorkCapacityList(
                typedStatement.getSjukskrivningsgrad(),
  ...
  private static SickLeaveCertificateBuilder buildSickLeaveCertificateCommon(
      Certificate certificate, GrundData grundData, ImmutableList<Diagnos> diagnoses) {
    return new SickLeaveCertificateBuilder(certificate.getId())
        ...
        .diagnoseCode(
            (diagnoses != null && !diagnoses.isEmpty()) ? diagnoses.get(0).getDiagnosKod() : null)
        .secondaryDiagnoseCodes(buildSecondaryDiagnoseCodes(diagnoses));
  }

  private static List<WorkCapacity> buildWorkCapacityList(
      String workCapacityDegree, InternalLocalDateInterval sickLeavePeriod) {
  ...
  private static List<WorkCapacity> buildWorkCapacityList(
      ImmutableList<Sjukskrivning> sickLeaveList) {
    return sickLeaveList.stream()
        .map(CertificateToSickLeaveCertificateConverter::getWorkCapacityReduction)
