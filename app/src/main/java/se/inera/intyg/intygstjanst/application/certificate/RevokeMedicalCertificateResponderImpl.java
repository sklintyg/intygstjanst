...
public class RevokeMedicalCertificateResponderImpl
    implements RevokeMedicalCertificateResponderInterface {

  private static final Logger LOGGER =
      LoggerFactory.getLogger(RevokeMedicalCertificateResponderImpl.class);
  ...
  @Autowired protected SjukfallCertificateService sjukfallCertificateService;
  ...
  public RevokeMedicalCertificateResponseType revokeMedicalCertificate(
    ...
    try {
      ...
      Certificate certificate =
          certificateService.revokeCertificate(personnummer.orElse(null), certId);
      ...
      if (!certificate.isTestCertificate()) {
        certificate.getStates().stream()
            ...
            .forEach(
                recipient ->
                    senderService.sendCertificateRevocation(
                        certificate, recipient, request.getRevoke()));
      }
      ...
      LOGGER.info(
          "Tried to revoke certificate '"
              ...
              + "' for patient '"
              + pnrHash
              + "' but certificate does not exist");
      response.setResult(
      ...
      LOGGER.info(
          "Tried to revoke certificate '"
              ...
              + "' for patient '"
              + pnrHash
              + "' which already is revoked");
      response.setResult(
          ResultOfCallUtil.infoResult("Certificate '" + certId + "' is already revoked."));
      return response;

    } catch (CertificateValidationException e) {
      LOGGER.info(
          VALIDATION,
          "Validation error found for revoke certificate '"
              + certId
              + "' issued by '"
              + safeGetIssuedBy(request)
              + "' for patient '"
              + pnrHash
              + ": "
              + e.getMessage());
      ...
      LOGGER.warn(
          "Encountered an exception when sending a revocation to subsystem '"
              + e.getSubsystemId()
              + "'");
      response.setResult(
          ResultOfCallUtil.failResult(
  ...
  protected String safeGetCertificateId(RevokeMedicalCertificateRequestType request) {
    ...
    if (request.getRevoke() != null
        && request.getRevoke().getLakarutlatande() != null
  ...
  private String getPersonnummerHash(
      Optional<Personnummer> personnummer, RevokeMedicalCertificateRequestType request) {
